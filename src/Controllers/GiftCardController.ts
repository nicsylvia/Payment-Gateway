...
export const BusinessGiftCard = AsyncHandler(async(
    req: Request,
    res: Response,
    next: NextFunction
) =>{
    const Business = await BusinessModels.findById(req.params.businessID).populate({
        path: "giftCard"
    });
    ...
    return res.status(200).json({
        message: `Successfully got all gift cards generated by ${Business?.name}`,
        data: Business
    })
})